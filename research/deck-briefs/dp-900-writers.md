# DP-900: notes for every writer (read after DIGEST.md)

- **Slug:** `dp-900`. **Family prefix:** `dp900` (card ids `dp900.<topic-slug>.<card-slug>`, concept ids `dp900.<concept-slug>`). Use the concept ids in `dp-900-concepts.json`; add new ones only when a card needs them.
...
- **Evidence is required.** The texts are in `research/sources/dp-900/`; the slice prints a passage under each concept.
