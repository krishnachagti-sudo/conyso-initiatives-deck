...
| **B. Open licence** | CC BY, CC BY-SA, public domain, or a permissive software licence (Apache 2.0, MIT) on documentation | Quote and adapt, with attribution per the licence | Kubernetes docs and the CNCF curriculum (CC BY 4.0); Google Cloud docs pages that carry the CC BY 4.0 footer; Microsoft's azure-docs repository (CC BY 4.0); the Scrum Guide (CC BY-SA 4.0); US federal works such as NIST publications, FAA handbooks, IRS publications and the Internal Revenue Code; EU legislation; Prometheus documentation (Apache 2.0), keeping the licence notice with the attribution; Helm documentation (helm.sh footer: CC BY 4.0) |
| **C. Facts only** | Readable, but licensed for personal use or all rights reserved | Facts in our own words, linked to the page, never copied. Facts are not protected; wording is. The one court precedent we found, *ETS v. Katzman* (3d Cir. 1986), declined to stop a coaching school "testing the same concept in the same order, as long as it does not use the same or substantially similar language". | AWS whitepapers that carry their own "All rights reserved" notice (docs.aws.amazon.com pages themselves are tier B: the AWS Site Terms, updated 4 June 2025, license them under CC BY-SA 4.0 with code under MIT-0; a person should confirm this reading before an AWS deck is released); Cisco docs; SEC rule pages (paraphrase the rule; cite the rule number); Microsoft Learn pages outside openly licensed repositories; the FAA question bank (confidential), so FAA decks come from the handbooks instead; state CDL manuals (© AAMVA), so CDL decks come from 49 CFR |
| **D. Never** | Exam content, proprietary curricula, anything whose terms forbid derivative use | Not used at all | Everything in §2, plus Salesforce Trailhead content (treated as confidential by its programme terms), Snowflake and Databricks docs (site terms forbid derivative works), MSRB pages (its terms forbid derivative works), FINRA pages (its Terms of Use bar using the site with any machine-learning or AI program; FINRA rules are cited from their SEC or Federal Register publication instead), IAPP pages (its Conditions of Use bar using the site with any AI system; privacy decks rest on the laws themselves), CompTIA pages (its terms bar transmitting its content into AI tools), AICPA and NASBA pages (their terms object to AI use and bar automated access), aws.amazon.com (its Site Terms bar derivative use; docs.aws.amazon.com is CC BY-SA 4.0 and usable) and nasaa.org (its robots.txt blocks automated access) |
