...
| **D. Never** | Exam content, proprietary curricula, anything whose terms forbid derivative use | Not used at all | Everything in §2, plus Salesforce Trailhead content (treated as confidential by its programme terms), Snowflake and Databricks docs (site terms forbid derivative works), MSRB pages (its terms forbid derivative works), FINRA pages (its Terms of Use bar using the site with any machine-learning or AI program; FINRA rules are cited from their SEC or Federal Register publication instead) and nasaa.org (its robots.txt blocks automated access) |
